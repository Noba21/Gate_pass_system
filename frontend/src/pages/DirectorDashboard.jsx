...
function DirectorDashboard() {
  ...
  const loadRequests = async () => {
    ...
    setError('');
    try {
      const res = await api.get('/gatepasses/director/pending');
  ...
  const handleAction = async (id, action) => {
    try {
      ...
      if (action === 'approve') {
        await api.post(`/gatepasses/director/${id}/approve`, { note });
      } else {
        await api.post(`/gatepasses/director/${id}/reject`, { note });
      }
      await loadRequests();
    } catch (err) {
  ...
  return (
    <div>
      <h2 className="h5 fw-semibold mb-4">Director - Requests Verified by Store</h2>
      <div className="card shadow-sm">
        <div className="card-body">
          {loading && <div className="small text-muted">Loading...</div>}
          {error && <div className="small text-danger mb-2">{error}</div>}
          {!loading && !requests.length && (
            <div className="small text-muted">No requests awaiting director approval.</div>
          )}
          {requests.map((r) => (
            <div
              key={r.id}
